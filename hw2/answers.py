...
1)  1. for a computation defined as above, we can compute the derivative of f_i and immediately use the computation and calculate f_i+1 then freeing f_i
this is possible due to the fact that in forward mode AD, the gradient information at each step only depends on the previous step. theoritically, this is O(1) memory complexity.
    2. we cannot reduce less that O(n), but we can reduce physically by checkpointing. the concept is to store only a small number of intermediate computations during the forward pass.
    Then, during the backward pass, recompute the necessary intermediate values on-the-fly from these checkpointed values. 
2) yes, in forward AD, e differentiation operation traverses the computational graph in the same direction as the computation itself.
    Each node in the graph represents a mathematical operation, and each edge represents a variable or intermediate result.
    The derivative of each operation with respect to its inputs can be computed using the chain rule,
    and these derivatives are then combined to give the derivative of the overall function.
    as for backward AD, every step is the same but the backward AD traverses the graph from the opposite direction to the computation.
3) Backpropogation on Resnets and VGG's have very high memory problem during training, thus checkpointing can greatly reduce the memory overhead.
"""
